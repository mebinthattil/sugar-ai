...
|----------|---------|--------------|----------|
| `/ask` | RAG-enabled Q&A for kids | Query parameter | • Retrieval-Augmented Generation<br>• Sugar/Pygame/GTK documentation<br>• Child-friendly responses |
| `/ask-llm` | Direct LLM without RAG | Query parameter | • No document retrieval<br>• Direct model access<br>• Faster responses |
| `/ask-llm-prompted` | Custom prompt with advanced controls | JSON body | • Custom system prompts<br>• Configurable generation parameters<br>• Maximum flexibility |
...
- **Advanced POST endpoint (/ask-llm-prompted)**
    ...
    - Uses your own custom system prompt (no built-in Sugar/kids-specific prompting)
    - Accepts JSON request body with configurable parameters
    - Provides direct LLM access without RAG (Retrieval-Augmented Generation)
    ...
    **Use Cases:**
    - **Code Generation**: Use prompts like "You are a Python expert. Write clean, commented code."
    - **Creative Writing**: Use prompts like "You are a creative writing assistant. Write engaging stories."
    - **Technical Documentation**: Use prompts like "You are a technical writer. Explain concepts clearly and concisely."
    - **Educational Content**: Use prompts like "You are a teacher. Explain complex topics in simple terms."

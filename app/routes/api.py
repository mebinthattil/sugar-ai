...
class PromptedLLMRequest(BaseModel):
    ...
    temperature: float = Field(0.7, description="Temperature for sampling (0.0 = deterministic)")
...
async def ask_llm_prompted(
    ...
    client_ip = request.client.host if request else "unknown"
    logger.info(f"REQUEST - /ask-llm-prompted - User: {user_info['name']} - IP: {client_ip} - Question: {request_data.question[:50]}...")
